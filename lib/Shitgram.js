...
module.exports = class Shitgram {
	 ...
	 */
	constructor(settings = {}, BASE_URL = 'https://www.instagram.com', API_URL = 'https://i.instagram.com/api/v1') {
		this.username = settings.username || false;
		this.password = settings.password || false;
		this.sessionID = settings.sessionID || false;
	...
	async user(username, options = {}) {
		...
		try {
			...
			if (sessionID) {
				const data_ws = await this.getUserDataWithSession(data.graphql.user.id, sessionID);
			...
			throw {
				method: error.hasOwnProperty('config') ? error.config.method.toUpperCase() : undefined,
				statusCode: error.hasOwnProperty('response') ? error.response.status : undefined,
	...
	async story(username, options = {}) {
		...
		try {
			...
			if (sessionID) {
				...
				const data_st = await this.getUserStoriesWithSession(userID, sessionID);
				const data_ws = await this.getUserDataWithSession(userID, sessionID);
	...
	async highlight(mediaID, options = {}) {
		...
		try {
			...
			if (sessionID) {
				const data_hl = await this.getUserHighlightsWithSession(userID, sessionID);
				const data_ws = await this.getUserDataWithSession(userID, sessionID);
				...
				tray.filter(function(highlight) {
					...
					function dataHandler(data, highlights) {
						return {
							...
							highlights: highlights ? options.exclude !== undefined ? highlights.filter((highlight) => highlight.mediaURL.split('?')[0].indexOf(ExcludeType[options.exclude.toUpperCase()]) === -1) : highlights : highlights,
	...
	async image(mediaCode, options = {}) {
		...
		try {
			...
			if (sessionID) {
				const data_ws = await this.getUserDataWithSession(data.graphql.shortcode_media.owner.id, sessionID);
	...
	async album(albumCode, options = {}) {
		...
		try {
			...
			if (error.message === `Cannot read property 'edges' of undefined`) {
	...
	_get(url, options = {}) {
		...
		options.headers['User-Agent'] = 'Mozilla/5.0 (Linux; Android 8.1.0; motorola one Build/OPKS28.63-18-3; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/70.0.3538.80 Mobile Safari/537.36 Instagram 72.0.0.21.98 Android (27/8.1.0; 320dpi; 720x1362; motorola; motorola one; deen_sprout; qcom; pt_BR; 132081645)';
		
		return require('node-fetch')(url, options).then((response) => response.json()).catch((error) => error);
	}
};