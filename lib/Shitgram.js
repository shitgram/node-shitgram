...
const ExcludeType = require('./ExcludeType.js');
...
module.exports = class Shitgram {
	...
	getDataWithSessionID(userID, sessionID) {
		return axios({
			method: 'GET',
	...
	async user(username, options) {
		...
		try {
			...
			if (sessionID) {
				const { data: data_ws } = await this.getDataWithSessionID(data.graphql.user.id, sessionID);
	...
	async story(username, options) {
		...
		try {
			...
			if (sessionID) {
				const { data: data_ws } = await this.getDataWithSessionID(data.user.pk, sessionID);

				if (options.defaultResponse) {
					return data;
				} else {
					return RewriteObjects(objectStory(data), 'author.avatarURL', data_ws.user.hd_profile_pic_url_info.url);
				}
			} else {
				if (options.defaultResponse) {
					return data;
	...
	async image(mediaCode, options) {
		...
		try {
			...
			if (sessionID) {
				const { data: data_ws } = await this.getDataWithSessionID(data.graphql.shortcode_media.owner.id, sessionID);
